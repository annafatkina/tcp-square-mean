...
class Dumper : public std::enable_shared_from_this<Dumper> {
    ...
    // If dumper is not shutdown and there is any logs stored, dump logs to the file specifed in ctor.
    void work();

    // Dump data stored in the specified 'dumpQueue' to a file specified in ctor.
