    ...
    auto callback = [this, self](boost::system::error_code ec, int size) {
        if (!ec) {
            try {
                ...
                logger_->printError("Error while calling 'Session::do_read()': ",
                    e.what());
            }
        } else {
            logger_->printError("Error while writing data to client:\n\t", std::string(ec.message()),
            "\nfor session with session id ", sessionId_,". Closing this session." );
        }
    };
...
// public
Session::Session(Context& io_context, Tcp::socket socket, int sessionId, std::shared_ptr<Logger> logger,
...
createSession(boost::asio::io_context &    io_context,
              boost::asio::ip::tcp::socket socket, int sessionId, std::shared_ptr<Logger> logger, std::shared_ptr<IComputer> computer) {
    try {
