...
TcpServer::do_accept() {
    acceptor_.async_accept([this](boost::system::error_code ec,
        ...
        if (!ec) {
            try {
                sessionFactory_(context_, std::move(socket), sessionCounter_, logger_, computer_)
                    ->start();
                sessionCounter_++;
            } catch (const std::exception &e) {
                logger_->printError("Failed to create session with a given session ",
                             "factory method, error: ",
                            e.what());
            }
        } else {
            logger_->printError("Failed to accept connection, error: ", ec.message());
        }
...
TcpServer::TcpServer(short port, SessionFactoryFunc sessionFactoryFunc)
    ...
    loggerThread_ = std::make_shared<std::thread>(
        boost::bind(&Logger::run, logger_.get()));
    do_accept();
}
